...
import sql_queries
from helper_functions_correct import append_df_to_excel, write_to_csv, Regional_ATP_rewrite, Regional_Replenishment_rewrite
...
def main(today_date, country_list, data_queried, files_processed, replenishment_done, reports_written, summary_completed):
    # Processing for each country
    ATP_reports_path = {'MY':'', 'TH':'','TW':'','ID':'','VN':'','PH':''}
    Replenishment_reports_path = {'MY':'', 'TH':'','TW':'','ID':'','VN':'','PH':''}
    ...
    for country in country_list:
        ...
        if not data_queried:
            write_to_csv(sql_queries.get_cogs(country, TODAY), file_names['cogs'])
            write_to_csv(sql_queries.get_purchase_type(country), file_names['purchase_type'])
            write_to_csv(sql_queries.get_brands(country, str_four_weeks_ago_date, str_two_weeks_ago_date, str_yesterday_date),file_names['brand'])
            write_to_csv(sql_queries.get_color(country, TODAY), file_names['new_color'])
            ...
            write_to_csv(sql_queries.get_replenishment_ext(country, str_four_weeks_ago_date, str_two_weeks_ago_date,str_yesterday_date), file_names['ext'])
        ...
        if not replenishment_done:
            ...
            brand = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'brand.csv'), encoding='utf-8-sig', index_col=False)
            cogs = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'cogs.csv'), encoding='utf-8-sig', index_col=False)
            new_color = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'new_color.csv'), encoding='utf-8-sig', index_col=False)
            replenishment = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'replenishment.csv'), encoding='utf-8-sig', index_col=False)
            ext = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'ext.csv'), encoding='utf-8-sig', index_col=False)
            pr_po_inbound = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'pr_po_inbound.csv'), encoding='utf-8-sig', index_col=False)
            item_profile_history = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'item_profile_history.csv'), encoding='utf-8-sig', index_col=False)
            cat_cluster = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'cat_cluster.csv'), encoding='utf-8-sig', index_col=False)
            cfs_wk2 = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'cfs_wk2.csv'), encoding='utf-8-sig', index_col=False)
            cfs_wk4 = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'cfs_wk4.csv'), encoding='utf-8-sig', index_col=False)
            cfs_ytd = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'cfs_ytd.csv'), encoding='utf-8-sig', index_col=False)
            purchase_type = pd.read_csv('{}'.format(input_file_path+'/'+country+'_'+'purchase_type.csv'), encoding='utf-8-sig', index_col=False)

            print("All files read!")

            ext = ext.sort_values('sourcing_status').drop_duplicates(['sku_id', 'grass_date'], keep='first')

            purchase_type, ext, replenishment, pr_po_inbound, new_color = process_raw_data_frames(purchase_type, ext, brand,
                                                                                                  replenishment,
                                                                                                  pr_po_inbound,
                                                                                                  new_color)
        ...
        if not reports_written:
            if replenishment_done:
                ...
                inbounded_tab = pd.read_csv(new_folder + "inbounded_tab.csv", encoding='utf-8-sig')
                rop_to_pr_tab = pd.read_csv(new_folder + "rop_to_pr.csv", encoding='utf-8-sig')
                ytd_fn = "{}/ytd_product_info.csv".format(country)
                ytd_atp_df = pd.read_csv(ytd_fn, na_values="n.a.", encoding='utf-8-sig')
                ytd_write = process_dfs_before_write(ytd_atp_df)
                wk_2_fn = "{}/w-2_product_info.csv".format(country)
    ...
    def find_file_path(pattern, root):
    ...
    if not summary_completed:
        ...
        if reports_written:
            for i in country_l:
                root='./'+i
                ATP_reports_path[i]=find_file_path('Report',root) #can change to the report date
                Replenishment_reports_path[i]=find_file_path('Tracker',root) #can change to the report date
        ...
        for c in range(len(ATP_reports_path)):
        ...
        for c in range(len(Replenishment_reports_path)):
...
if __name__ == '__main__':
    ...
    main(args.date, args.countries, args.queried, args.processed, args.rep_done, args.reports_written, args.summary_done)
